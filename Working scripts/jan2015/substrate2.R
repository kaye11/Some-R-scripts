...
M.gls<- gls(Form, data=sub, method="REML")
...
#M7.lme <- lme(Form, random = ~1|reptreat,  weights=varIdent(form=~1|reptreat), 
              correlation=corAR1 (form=~1|reptreat/treatment), method="REML", data=sub) # stick to this, same with M5
...
anova(M.gls, M1.lme, M2.lme, M3.lme, M4.lme, M5.lme, M7.lme, M9.lme)
...
anova(M2.Full, M2.B) #can be dropped
...
M4.B <- lme (CellsN~treatment*T*Bin, random = ~1|reptreat/Bin,  weights=varIdent(form=~1|treatment), 
             correlation=corAR1 (), method="REML", data=sub ) #best

anova(M4.final, M4.A, M4.B)

E2<-resid(M4.B,type="normalized")
...
BinA.gls<- gls(Form, data=BinA, method="REML")
...
#residuals
BinA.E2<-resid(BinA4.lme,type="normalized")
BinA.F2<-fitted(BinA4.lme)
op<-par(mfrow=c(2,2),mar=c(4,4,3,2))
MyYlab="Residuals"
...
summary(glht(M4.B, covariate_average=TRUE))

summary(glht(BinA3.lme, covariate_average=TRUE))
summary(glht(BinB3.lme, covariate_average=TRUE))
summary(glht(BinC3.lme, covariate_average=TRUE))


ggplot(data = BinA, aes(x=T,y=CellsN, color=treatment, shape=treatment))+ 
  stat_smooth(method="loess", formula=y~x, size=2, se=TRUE)+   scale_colour_manual(values = c("lightcoral", "seagreen3", "steelblue2"),
                                                               breaks=c("Control", "Ge","Si"),
                                                               labels=c("Control", "dGe", "dSi")) +
  labs(list(x = "Time (s)", y = "Normalized cell count"))+ labs (color="Experimental condition")+
  theme(axis.text=element_text(size=20), axis.title.y=element_text(size=20,face="bold", vjust=-0.0001), 
...
ggplot(data = sub, aes(x=T,y=CellsN, color=treatment, shape=treatment))+ 
  ...
  facet_grid(.~Bin, labeller=mf_labeller)+ scale_colour_manual(values = c("lightcoral", "seagreen3", "steelblue2"),
                                                               ...
                                                               labels=c("Control", "dGe", "dSi")) +
  labs(list(x = "Time (s)", y = "Normalized cell count"))+ labs (color="Experimental condition")+
  theme(axis.text=element_text(size=20), axis.title.y=element_text(size=30,face="bold", vjust=-0.0001), 
        axis.title.x=element_text(size=30,face="bold", vjust=-0.02),
        plot.title = element_text(size =20, face="bold"), axis.text=text,  legend.position="bottom",
        strip.text.x = element_text(size =25, face="bold"), strip.text.y = text, legend.title=element_text(size =25, face="bold"), 
        legend.text=element_text(size =25), panel.margin=unit (0.5, "lines"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), plot.margin = unit(c(1,1,1,1), "cm"))
...
ggplot(data = BinA, aes(x=T,y=CellsN, color=treatment, shape=treatment))+ 
  ...
  scale_colour_manual(values = c("lightcoral", "seagreen3", "steelblue2"),
                                                               breaks=c("Control", "Ge","Si"),
                                                               labels=c("Control", expression (Ge(OH)["4"]), expression (Si(OH)["4"]))) +
  labs(list(x = "Time (s)", y = "Normalized cell count"))+ labs (color="Experimental condition")+
  theme(axis.text=element_text(size=20), axis.title.y=element_text(size=30,face="bold", vjust=-0.0001), 
        axis.title.x=element_text(size=30,face="bold", vjust=-0.02),
        plot.title = element_text(size =20, face="bold"), axis.text=text,  legend.position="bottom",
        strip.text.x = element_text(size =25, face="bold"), strip.text.y = text, legend.title=element_blank(), 
        legend.text=element_text(size =25), panel.margin=unit (0.5, "lines"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), plot.margin = unit(c(1,1,1,1), "cm"))